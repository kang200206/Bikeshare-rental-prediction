...
class TransformerModel(nn.Module):
    def __init__(self, input_size, d_model, nhead, num_layers, output_length):
        ...
        # Output layer
        self.fc_out = nn.Linear(d_model, output_length)
    ...
    def forward(self, x):
        ...
        tgt = torch.zeros(x.size(0), self.output_length, self.d_model).to(x.device) 
...
# 初始化模型
model = TransformerModel(input_size, d_model, nhead, num_layers, output_length).to(device)
...
for experiment in range(num_experiments):
    ...
    # 保存模型
    model_path = os.path.join(model_dir, f"transformer_output_{output_length}_mse_{mse:.4f}_mae_{mae:.4f}_experiment_{experiment + 1}_{current_time}.pth")
    ...
    x_true = [i for i in range(0,336)]
    x_pred = [i for i in range(96,336)]
    plt.plot(x_pred, predictions_original[404+96+24], label="Predictions (Original Scale)", color="blue")
    plt.plot(x_true, (list(true_values_original[404+96-96+24][0:96])+list(true_values_original[404+96+24])), label="True Values (Original Scale)", color="orange")
