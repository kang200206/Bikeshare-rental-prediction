...
class TransformerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # 编码器输出
        memory = self.transformer_encoder(x)  # [batch_size, sequence_length, d_model]

        # 解码器输入：这里我们假设目标序列的起始时间步全为零，之后会根据解码器的输出逐步生成
        tgt = torch.zeros(x.size(0), self.output_length, self.d_model).to(x.device)  # [batch_size, output_length, d_model]
        output = self.transformer_decoder(tgt, memory)  # [batch_size, output_length, d_model]

        # 输出层
        output = self.fc_out(output)  # [batch_size, output_length, output_length]
        return  output[:, -1, :]
